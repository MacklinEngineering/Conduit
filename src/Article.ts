// File Not used, might parse out App.ts file into these separated files, but that's a TODO. This was from testing.
...
//   /** Get article */
//   async get(event: { pathParameters: { slug: any; }; }) {
...
//   /** Update article */
//   async update(event: { body: string; pathParameters: { slug: any; }; }) {
//     const body = JSON.parse(event.body);
...
//   /** Delete article */
//   async delete(event: { pathParameters: { slug: any; }; }) {
//     const authenticatedUser = await User.authenticateAndGetUser(event);
//     if (!authenticatedUser) {
//       return Util.envelop('Must be logged in.', 422);
//     }
...
//   async favorite(event: { pathParameters: { slug: any; }; httpMethod: string; }) {
//     const authenticatedUser = await User.authenticateAndGetUser(event);
//     if (!authenticatedUser) {
//       return Util.envelop('Must be logged in.', 422);
//     }
...
//           (        e: any) => (e !== authenticatedUser.username));
//       /* istanbul ignore next */
...
//   /** List articles */
//   async list(event: { queryStringParameters: {}; }) {
//     const authenticatedUser = await User.authenticateAndGetUser(event);
...
//   /** Get Articles feed */
//   async getFeed(event: { queryStringParameters: {}; }) {
//     const authenticatedUser = await User.authenticateAndGetUser(event);
//     if (!authenticatedUser) {
//       return Util.envelop('Must be logged in.', 422);
//     }
...
//       data.Items.forEach((item: { tagList: { values: any[]; }; }) => {
//         /* istanbul ignore next */
//         if (item.tagList && item.tagList.values) {
//           item.tagList.values.forEach((tag: string | number) => uniqTags[tag] = 1);
//         }
//       });
...
//  */
// async function queryEnoughArticles(queryParams: { TableName?: any; IndexName?: string; KeyConditionExpression?: string; ExpressionAttributeValues?: { ':dummy': string; } | { ':dummy': string; }; ScanIndexForward?: boolean; FilterExpression?: string; ExclusiveStartKey?: any; }, authenticatedUser: any,
...
//  */
// async function transformRetrievedArticle(article: { dummy: any; tagList: any[]; favoritesCount: number; favorited: boolean; favoritedBy: string | any[]; author: any; }, authenticatedUser: { username: any; } | undefined) {
//   delete article.dummy;
